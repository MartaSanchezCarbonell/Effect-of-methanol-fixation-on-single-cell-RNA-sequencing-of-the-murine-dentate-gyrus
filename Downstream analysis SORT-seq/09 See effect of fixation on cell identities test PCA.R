...
merged.pilot$Condition <- factor(pilot.integrated$Condition, levels=c("Fresh_cells","Fixed_cells"))
Idents(pilot.integrated) <- "Condition"

#Merging
merged.pilot<-Merge_Seurat_List(pilot.list)
merged.pilot
a<-VariableFeatures(object = Fresh_cells)
#Perform linear dimensional reduction
merged.pilot <- RunPCA(merged.pilot, features = VariableFeatures(object = merged.pilot))
# Examine and visualize PCA results a few different ways
DimPlot(merged.pilot, reduction = "pca")
#Determine the ‘dimensionality’ of the dataset
#merged.pilot <- JackStraw(merged.pilot, num.replicate = 100)
#merged.pilot <- ScoreJackStraw(merged.pilot, dims = 1:20)
#JackStrawPlot(merged.pilot, dims = 1:20)
ElbowPlot(merged.pilot)
...
merged.pilot <- FindNeighbors(merged.pilot, dims = 1:12, k.param=10)
merged.pilot<- FindClusters(merged.pilot, resolution = 0.5, algorithm=1)
...
pilot.markers <- FindAllMarkers(object=merged.pilot, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.001, test.use= "wilcox",assay="RNA")
pilot.markers %>%
  group_by(cluster) %>%
...
merged.pilot@active.ident <- plyr::mapvalues(x = merged.pilot@active.ident, from = current.cluster.ids, to = new.cluster.ids)
merged.pilot$CellType <- Idents(merged.pilot)

#UMAP
merged.pilot <- RunUMAP(merged.pilot, dims = 1:15, n.neighbors = 15)
# note that you can set `label = TRUE` or use the LabelClusters function to help label individual clusters
DimPlot(merged.pilot, reduction = "umap", label=FALSE, pt.size=2.3)

#Differential expressed genes heatmap (clusters)
pilot.markers %>%
  group_by(cluster) %>%
  slice_max(n =10, order_by = avg_log2FC) -> top10_log2FC
top10_log2FC_HM<-DoHeatmap(merged.pilot, features = top10_log2FC$gene, label=FALSE)
...
top10_pval_HM<-DoHeatmap(merged.pilot, features = top10_pval$gene, label=FALSE, assay = )
...
DefaultAssay(merged.pilot) <- "RNA"
FeaturePlot(merged.pilot, features = c("Lpar1", "Nes", "Prom1", "Hes5", "Ascl1", "Tfap2c", "Vnn1", "Wnt8b", "2700094K13Rik", 
...
merged.pilot$CellType <- Idents(merged.pilot)
Astrocytes_RGL<-subset(merged.pilot, subset = CellType=="Astrocytes +  RGL")
...
Idents(merged.pilot) <- "Condition"
DimPlot(merged.pilot, reduction = "umap", pt.size=2.3)
Idents(merged.pilot) <- "CellType"
DimPlot(merged.pilot, reduction = "umap", pt.size=2.3, split.by = "Condition")+theme(
...
Idents(merged.pilot) <- "Condition"
dittoBarPlot(merged.pilot,var="Condition", group.by = "CellType",retain.factor.levels=TRUE, color.panel=c("#F8766D","#00BFC4"))

#pie graphs with % of the different cell identities
## extract meta data
md <- merged.pilot@meta.data %>% as.data.table
...
pilot.list.int<-SplitObject(merged.pilot, split.by = "Condition")
...
merged.pilot <- FindNeighbors(merged.pilot, dims = 1:12, k.param=10)
merged.pilot<- FindClusters(merged.pilot, resolution = seq(from = 0.1, to = 1, by = 0.1), algorithm=1)
clustree(merged.pilot, prefix = "integrated_snn_res.")
#Playing with K
merged.pilot_K5 <- FindNeighbors(merged.pilot, dims = 1:12, k.param=5)
merged.pilot_K7 <- FindNeighbors(merged.pilot, dims = 1:12, k.param=7)
merged.pilot_K10 <- FindNeighbors(merged.pilot, dims = 1:12, k.param=10)
merged.pilot_K15 <- FindNeighbors(merged.pilot, dims = 1:12, k.param=15)
merged.pilot_K20 <- FindNeighbors(merged.pilot, dims = 1:12, k.param=20)

merged.pilot_K5<- FindClusters(merged.pilot_K5, resolution = 0.5, algorithm=1)
merged.pilot_K7<- FindClusters(merged.pilot_K7, resolution = 0.5, algorithm=1)
merged.pilot_K10<- FindClusters(merged.pilot_K10, resolution = 0.5, algorithm=1)
merged.pilot_K15<- FindClusters(merged.pilot_K15, resolution = 0.5, algorithm=1)
merged.pilot_K20<- FindClusters(merged.pilot_K20, resolution = 0.5, algorithm=1)



merged.pilot[["K5"]] <- merged.pilot_K5@meta.data[["seurat_clusters"]]
merged.pilot[["K7"]] <- merged.pilot_K7@meta.data[["seurat_clusters"]]
merged.pilot[["K10"]] <- merged.pilot_K10@meta.data[["seurat_clusters"]]
merged.pilot[["K15"]] <- merged.pilot_K15@meta.data[["seurat_clusters"]]
merged.pilot[["K20"]] <- merged.pilot_K20@meta.data[["seurat_clusters"]]

clustree(merged.pilot, prefix = "K")
